...
const AddMembers: React.FC<RouteComponentProps> = ({ history }) => {
    ...
    if (authenticationInformation && searchMail)
      fetchUserByEmail(authenticationInformation?.token, searchMail)
        .then((user) => {
          ...
          setSelectedGroups(
            owned?.groups.filter((grp) => user.groups.includes(grp?.id ?? ""))
          );
        })
  ...
  const onClickAddUser = () => {
    if (
      ...
      executeDelayed(100, () => history.push("/members"));
    }
  };
      ...
      return (
        <IonItem>
          ...
          <IonSelect
            ...
            okText="Gruppen passen!"
            onIonChange={(e) => setSelectedGroups(e.detail.value)}
          >
            {owned?.groups!.map((value) => {
              return (
                <IonSelectOption key={value.id} value={value}>
                  {value.description}
                </IonSelectOption>
              );
            })}
