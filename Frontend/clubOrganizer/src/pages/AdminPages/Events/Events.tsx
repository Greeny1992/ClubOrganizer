...
import { fetchAddGroupAction, fetchOwnedAction, fetchOwnedActions } from '../../../services/actions/club';
...
const Events: React.FC<RouteComponentProps> = ({ history }) => {
    ...
    const NoValuesInfo = () => !isLoading && owned?.groups.length == 0 ?
        (<IonCard>
    ...
    const ListEvents = () => {
        ...
        if (owned){
            const items = owned.events.map(value => {
                ...
                const zusagen = value.acceptUsers.length > 0 ? value.acceptUsers.map(x=> x.userName + ", ") : "keine Zusagen noch!"
                const absagen = value.cancelUsers.length > 0 ? value.cancelUsers.map(x=> x.userName + ", ") : "keine Absagen noch!"
                return (
                            <IonCard className='userCard' key={value.id} /* onClick={() => history.push('/events/edit/' +value.id)} */>
                                <IonCardHeader>
                                    <IonCardTitle>Event: {value.name}</IonCardTitle>
                                    <IonCardContent>
                                        <IonGrid>
                                            <IonRow> <IonCol>Beschreibung:</IonCol> <IonCol>{value.description}</IonCol></IonRow>
                                            <IonRow> <IonCol>Start:</IonCol> <IonCol>{'am '+ startDate + ' um ' + startTime}</IonCol></IonRow>
                                            <IonRow> <IonCol>Ende:</IonCol> <IonCol>{'am '+ endDate + ' um ' + endTime}</IonCol></IonRow>
                                            <IonRow> <IonCol>Aktiv:</IonCol> <IonCol>{value.active? "JA!" : "Nope"}</IonCol></IonRow>
                                            <IonRow> <IonCol>Zusagen:</IonCol> <IonCol>{zusagen}</IonCol></IonRow>
                                            <IonRow> <IonCol>Absagen:</IonCol> <IonCol>{absagen}</IonCol></IonRow>
    ...
    return (
        <IonPage>
            ...
            <IonRefresher slot="fixed" onIonRefresh={doRefresh}>
                ...
                {isLoading ? <IonItem><IonSpinner />Loading Events...</IonItem> : <ListEvents />}
                <IonToast
