...
endpoint.get<Club | ErrorMessage>(`${config.getClubControllerURI}GetClub?clubId=${id}`, { headers: createAuthenticationHeader(token) })
    ...
    endpoint.get<Club | ErrorMessage>(`${config.getUserURI}GetOwnedClub`, { headers: createAuthenticationHeader(token) })
        // Use this to simulate network latency
        //.then(r => executeDelayed(3000, () => r))
        .then(r => {
            if (r.status >= 300) {
...
endpoint.get<Clubs | ErrorMessage>(`${config.getUserURI}GetClubsFromUser`, { headers: createAuthenticationHeader(token) })
    // Use this to simulate network latency
    //.then(r => executeDelayed(3000, () => r))
    .then(r => {
        if (r.status >= 300) {
...
const _getActiveClub = () => Promise.resolve(Storage.get({key:'activeClub'}))
